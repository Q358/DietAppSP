    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.innerContainer}>
          ...
          <Text>{Math.round(servings.number_of_units)} serving{servings.number_of_units > 1 && "s"} per container</Text>
          ...
          <View style={{justifyContent:"space-between", flexDirection:"row", alignItems:"stretch"}}>
            <View>
              ...
              <Text style={{fontFamily:"fontBold", fontSize:40}}>Calories</Text>
            </View>
            <Text style={{fontFamily:"fontBold", fontSize:60}}>{servings?.calories}</Text>
          </View>
          <Divider width={10} style={{marginVertical:5}} color="black"/>
          <View style={{width:"100%", flexDirection:"row",justifyContent:"flex-end"}}>
            <Text style={{fontFamily:"fontBold"}}>% Daily Value</Text>
          </View>
          ...
          <FactRow title={`Includes ${Math.round(servings.added_sugars)}g Added Sugars`} indent={2}/>
    ...
    <>
      ...
      <View style={{flexDirection:"row", justifyContent:"space-between", backgroundColor:highlight}}>
        <View style={{flexDirection:"row", alignItems:"center"}}>
          <Text style={{fontFamily: bold ? "fontBold" : "fontRegular", marginRight:5, marginLeft: indent ? indent * 10 : 0}}>{title}</Text>
          <Text style={{fontFamily:"fontRegular"}}>{value}</Text>
        </View>
        {!noPercent && <Text style={{fontFamily: lightPercent ? "fontRegular" : "fontBold"}}>{percent}%</Text>}
      </View>
    </>
  )
}
